...
void FeatureTracker::setMask()
{
    ...
    // pair中含有pair。
    // 结构： cnt_pts_id.first = 追踪次数，cnt_pts_id.second.first = 追踪的坐标点  cnt_pts_id.second.second = 追踪点id
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (EQUALIZE)
    {
        ...
        // ! opencv 函数看一下
        // createCLAHE 自适应直方图均衡
        // 能有效的增强或改善图像（局部）对比度，从而获取更多图像相关边缘信息有利于分割。
        // 还能够有效改善 AHE 中放大噪声的问题。
        // 另外，CLAHE 的有一个用途是被用来对图像去雾。
    ...
    // 用于保存当前的图像帧的特征点信息
    // forw_pts 在cv::calcOpticalFlowPyrLK传入时为空，输出时候为跟上的点
    forw_pts.clear();

    if (cur_pts.size() > 0)    // 上一帧有特征点，就可以进行光流追踪了,根据这一贞的匹配信息，对上一贞信息进行瘦身
    {
        TicToc t_o;
        vector<uchar> status;
        vector<float> err;
        ...
        // 光流跟踪假设：1.亮度恒定， 像素的灰度值不会随着贞的跟踪改变
        // maxlevel 表示金字塔层数-1, i为0表示不使用金字塔
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (n_max_cnt > 0)
        {
            ...
            // 只有发布才可以提取更多特征点，同时避免提的点进mask // mask=0的点忽略
...
void FeatureTracker::rejectWithF()
{
    ...
    if (forw_pts.size() >= 8)
    {
        ...
        // 之所以使用虚拟相机，就是因为这里使用本质矩阵进行外点剔除 ，这样不影响输入的参数 F_THRESHOLD，通用性更强
